...
/**
 * @brief General log module name exposed to the user, so the logger can be used by user code if needed
 * 
 */
...
class Helpers
{
	public:
		...
		/**
		 * @brief Set the Mlog level for a module. Note that the global lov level can override a module log level
		 * 
		 * @param module 
		 * @param level 
		 */
		void setModuleLogLevel(std::string& module, IotexLogLevel level);
};
} // namespace iotex
